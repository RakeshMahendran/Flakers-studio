...
class IngestionJob(Base):
    ...
    @property
    def pages_processed_count(self) -> int:
        """Return completed page work from persisted counters."""
        return int(self.urls_processed or 0) + int(self.urls_completed or 0)

    @property
    def chunks_created_count(self) -> int:
        """Return the known chunk total."""
        return int(self.total_chunks_created or 0)

    @property
    def can_restart(self) -> bool:
        """Failed and cancelled jobs can be retried via a fresh run."""
        return self.status in {JobStatus.FAILED.value, JobStatus.CANCELLED.value}

    @property
    def can_cancel(self) -> bool:
        """Queued and running jobs can still receive a cancellation request."""
        return self.status in {JobStatus.QUEUED.value, JobStatus.RUNNING.value} and not self.cancellation_requested
