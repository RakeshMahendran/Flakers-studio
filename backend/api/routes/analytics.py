...
from backend.models.content import ContentChunk, IngestionJob
...
async def get_system_stats(
    ...
    try:
        ...
        assistant_stats = await db.execute(
            ...
            .select_from(Project)
            .join(Assistant, Assistant.project_id == Project.id, isouter=True)
            .where(Project.tenant_id == current_tenant.id, Project.status != ProjectStatus.DELETED)
        )
        ...
        chat_stats = await db.execute(
            ...
            .join(ChatSession, ChatSession.assistant_id == Assistant.id, isouter=True)
        ...
        processing_time_stats = await db.execute(
            ...
            .where(Assistant.tenant_id == current_tenant.id, ChatMessage.processing_time_ms.isnot(None))
        )
...
async def get_usage_analytics(
    ...
    try:
        ...
        top_assistants_result = await db.execute(
            ...
            .join(ChatSession, ChatSession.assistant_id == Assistant.id)
            .join(ChatMessage, ChatMessage.session_id == ChatSession.id)
            .where(Assistant.tenant_id == current_tenant.id, ChatMessage.created_at >= start_date)
...
async def get_assistant_analytics(
    ...
    try:
        ...
        jobs_result = await db.execute(
            select(IngestionJob)
            .where(IngestionJob.assistant_id == assistant_id, IngestionJob.tenant_id == current_tenant.id)
            .order_by(desc(IngestionJob.started_at))
            .limit(5)
        )
...
async def analytics_health_check(
    ...
    try:
        ...
        test_result = await db.execute(select(func.count(Assistant.id)).where(Assistant.tenant_id == current_tenant.id))
