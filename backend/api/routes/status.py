...
async def cancel_job(
    ...
    try:
        cancelled = await request_job_cancellation(db, job_id, reason)

        if not cancelled:
            raise HTTPException(status_code=400, detail="Failed to cancel job - job may already be completed")
...
async def system_health_check(
    ...
    try:
        ...
        try:
            from backend.vector_providers.qdrant_provider import get_qdrant_client
