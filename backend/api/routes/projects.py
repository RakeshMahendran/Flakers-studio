...
from backend.ingestion.project_deletion import ProjectDeletionService
from backend.models.assistant import Assistant, AssistantStatus, AssistantTemplate, SourceType
...
async def list_scraped_urls(
    ...
    job_result = await db.execute(
        select(IngestionJob).where(IngestionJob.id == job_uuid, IngestionJob.tenant_id == current_tenant.id)
    )
...
async def get_scraped_url_content(
    ...
    result = await db.execute(
        select(IngestionURL)
        .join(IngestionJob, IngestionJob.id == IngestionURL.job_id)
        .where(
            IngestionURL.job_id == job_uuid,
...
async def list_projects(
    ...
    try:
        ...
        query = query.where(Project.tenant_id == current_tenant.id)
...
async def get_project(
    ...
    try:
        ...
        result = await db.execute(
            select(Project).where(Project.id == project_uuid, Project.tenant_id == current_tenant.id)
        )
...
async def delete_project(
    ...
    try:
        result = await ProjectDeletionService.initiate_deletion(
            db, project_id, str(current_tenant.id)
        )
...
async def complete_project_deletion(
    ...
    try:
        project_result = await db.execute(
            select(Project).where(Project.id == uuid.UUID(project_id), Project.tenant_id == current_tenant.id)
        )
