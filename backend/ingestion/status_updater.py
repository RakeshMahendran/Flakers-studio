...
def build_job_status_payload(job: IngestionJob) -> Dict[str, Any]:
    urls_total = int(job.total_urls_discovered or 0)
    pages_processed = job.pages_processed_count
    chunks_created = job.chunks_created_count
    chunks_uploaded = int(job.chunks_uploaded or 0)

    if chunks_created > 0 and (job.current_stage in ["ingestion", "storing", "completed"]):
        progress_percentage = int(round((chunks_uploaded / chunks_created) * 100))
    else:
        progress_percentage = int(round((pages_processed / urls_total) * 100)) if urls_total > 0 else 0

    retryable = job.status in ["failed", "cancelled"] and (bool(job.error_details) or bool(job.cancellation_requested))
    ...
    return {
        ...
        "urls_scraped": int(job.urls_scraped or 0),
            ...
            async with AsyncSessionLocal() as db:
                ...
                for job in jobs:
                    if job.status == "completed":
                        completed_jobs += 1
                        total_chunks += job.chunks_created_count
                        total_pages += job.pages_processed_count
                    elif job.status in ["failed", "cancelled"]:
                        failed_jobs += 1
                    elif job.status in ["running", "queued"] or job.current_stage in ["discovery", "scraping", "processing", "embedding", "ingestion", "storing", "discovery_complete"]:
                        running_jobs += 1
